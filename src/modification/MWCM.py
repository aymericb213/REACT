...
class MWCM:
    ...
    def __init__(self, distance_matrix, constrained_partition, constraints, sat_rate=1, k_supp=0):
        ...
        self.soft = {key: [] for key in constraints}
        self.compelled = {key: [] for key in constraints}
        self.sat_rate = sat_rate
        self.modifications = {}
        self.batch = {}

    @timer
    def solve(self, mcs=False, verbose=False):
        """
        ...
        new_partition, cst_satisfaction, b, v, w = self.cop_vars()
        ...
        user_constraints = deepcopy(model.constraints)
        ...
        #model += sum(cst_satisfaction) >= (constraint_count - sum([len(self.compelled[key]) for key in self.compelled]))
        if not mcs:
            sat_constraint = sum(cst_satisfaction) >= (constraint_count - sum([len(self.soft[key]) for key in self.soft]))
            model += sat_constraint
            ...
            weights = [np.random.randint(1001)/1001 for _ in range(len(cst_satisfaction))]
            model.minimize(sum(weights * (1 - cst_satisfaction)))
        ...
        if s.solve(**params):
            if not mcs:
                ...
                print(f"Computed within {time() - ts} s")
                print(cst_satisfaction.value())
                mcs = [user_constraints[i] for i in range(len(cst_satisfaction)) if not cst_satisfaction[i].value()]
                mss = [user_constraints[i] for i in range(len(cst_satisfaction)) if cst_satisfaction[i].value()]
                print(f"Minimal correction set : {mcs}")
                #print(f"Maximal satisfiable set : {mss}")
        else:
            if not mcs:
                print("couldn't find solution, finding minimal conflict set")
                self.solve(mcs=True)
            else:
                print("couldn't find minimal conflict set")
    ...
    def cop_vars(self):
        ...
        if sum([len(v) for v in self.constraints.values()]) == 1:
            ...
            w = [intvar(0, np.matrix(self.p_dists_reps).max(), name="W")]
        else:
            cst_satisfaction = boolvar(shape=sum([len(self.constraints[key]) for key in self.constraints]), name="S")
            w = intvar(0, np.matrix(self.p_dists_reps).max(), shape=sum([len(self.constraints[key]) for key in self.constraints]), name="W")

        return new_partition, cst_satisfaction, b, v, w
    ...
    def user_constraints(self, model, new_partition, cst_satisfaction, verbose=False):
        ...
        for key in self.constraints:
            for constraint in self.constraints[key]:
                ...
                if constraint in self.compelled[key]:
                    model += sat == True
                if constraint in self.soft[key]:
                    value = self.p_dists_reps.iloc[constraint[0], self.constrained_partition[constraint[0]]] + self.p_dists_reps.iloc[constraint[1], self.constrained_partition[constraint[1]]]
                    model += (sat == False) == (weight == value)
                    model += (sat == True) == (weight == 0)
                match key.lower():
                    case "label":
                    ...
                    case "span":  # ((a,b...n), {k1, k2...kn})
                        ...
                        if type(constraint[1]) == set:  # specific case
                            ...
                            for c in range(self.k):
                                ...
                                model += (Count(group, c) > 0) == nb_clust[c] == sat
    ...
    def soften(self, key, constraint):
        """
        Soften a constraint.
        ...
        ----------
        constraint : tuple
            Constraint to soften.

        Returns
        -------
        None
        """
        if constraint in self.constraints[key]:
            self.soft[key].append(constraint)
        else:
            raise ValueError(f"Cannot soften constraint {constraint}, not found in {key} constraints")

    def compel(self, key, constraint):
        """
        Compel a constraint.
        ...
        """
        if constraint in self.constraints[key]:
            self.compelled[key].append(constraint)
        else:
            raise ValueError(f"Cannot compel constraint {constraint}, not found in {key} constraints")
